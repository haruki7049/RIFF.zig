# Pull Request & Commit Workflow for `RIFF.zig`
...
| **Nix Flake Check** | `nix flake check --all-systems` | Mirrors `nix-checker.yml` CI; run when `flake.nix`/`flake.lock` change |

If a change touches or adds fixtures under `src/assets/`, first run `git lfs pull` to make sure local LFS pointers are resolved before testing.

## 2. Commit & PR Title Conventions

Use Conventional Commits style prefixes, optionally with a scope:

- `feat:` New capability or supported chunk/format.
